...
class NWBDataset(Dataset):
    def __init__(
        ...
        channels_to_keep=None, very_bad_elecs=[234, 235], debug_mode=False
    ):
            ...
            with h5py.File(
                ...
                if data_type.lower() == 'hg':
                    raw_data = f['processing']['ecephys']['LFP']\
                        ['high gamma (CAR 200.0Hz)']['data'][:]
                elif data_type.lower() == 'lfc':
                    raw_data = f['processing']['ecephys']['LFP']\
                        ['preprocessed LFC (CAR 200.0Hz)']['data'][:]
                    ...
                    if ii == 0:
                        self.very_bad_elecs.extend(
                            [x + 256 for x in self.very_bad_elecs]
                        )
                else:
